...
if (game_is_running()) {
	...
	var sine = dsin(chain_swing_direction);
	var cosine = dcos(chain_swing_direction);
	for (var n = 0; n < total_chain_links; ++n) {
		with (chain_link[n]) {
			x = other.x + (sine * other.chain_link_distance[n]);
			y = other.y + other.chain_base_distance + (
				cosine * other.chain_link_distance[n]
			);
		}
	}
	with (chain_ball) {
		x = other.x + (sine * other.chain_ball_distance);
		y = other.y + other.chain_base_distance + (
			cosine * other.chain_ball_distance
		);
	}
	switch (state) {
	case "appearing":
	...
	case "centering":
		...
		if (x <= camera_left + CAMERA_WIDTH * 0.5) {
			x_speed = 0;
			y_speed = 0;
			chain_base = instance_create_depth(
				x, y, depth + 2, GHZBossChainBase
			);
			for (var n = 0; n < total_chain_links; ++n) {
				chain_link[n] = instance_create_depth(
					x, y, depth + 2, GHZBossChainLink
				);
			}
			chain_ball = instance_create_depth(
				x, y, depth + 1, GHZBossChainBall
			);
			state = "deploying";
