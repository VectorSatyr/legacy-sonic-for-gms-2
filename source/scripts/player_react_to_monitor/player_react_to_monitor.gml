function player_react_to_monitor(argument0, argument1) {
	var ind = argument0;
	var phase = argument1;
	...
	case "checking":
	    if ((spinning or state == player_is_gliding or state == player_is_glide_sliding) and
	        game_pc_in_shape(id, ind, x_wall_radius, y_radius)) {
	...
	case "entering":
	    if (not on_the_ground) {
	        if (y_speed < 0 and game_pc_upper_in_shape(id, ind, y_radius)) {
	            y_speed = 0;
	            ind.y_speed = -2;
	            ind.gravity_force = 0.21875;
	            break;
	        } else if (y_speed >= 0 and game_pc_lower_in_shape(id, ind, y_radius)) {
	            y_speed = -y_speed;
