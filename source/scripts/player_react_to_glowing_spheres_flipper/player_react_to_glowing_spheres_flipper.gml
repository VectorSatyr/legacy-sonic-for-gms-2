function player_react_to_glowing_spheres_flipper(argument0, argument1) {
	var ind = argument0;
	var phase = argument1;

	switch (phase) {
	case "checking":
	    if ((not on_the_ground and y_speed >= 0 and game_pc_lower_in_shape(id, ind, y_radius + 1)) or
	...
	case "entering":
		if (ground_id != ind) {
			game_pc_land(id, ind);
			var new_dir = game_pc_calc_shape_normal(id, ind, mask_direction);
